...
Where 
$$
c_i = \frac{(x_i - \bar{x}) }{S_{xx}} = \frac{(x_i - \bar{x}) }{\sum_{j=1}^n (x_j - \bar{x})^2 }.
$$


where $\sum(x_i - \bar{x})^2 = S_{xx}$
...
&= \beta_1 \sum_i c_i \frac{(x_i - \bar{x}) }{S_{xx}}  \\
