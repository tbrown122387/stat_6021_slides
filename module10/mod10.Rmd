...
In **multinomial logistic regression**, you have $\mathbf{x}_i^\intercal \in \mathbb{R}^p$, but now you have more $p$-dimensional coefficient vectors because you have more categories.
...
- set $\mathbf{c} = \boldsymbol{\beta}^0$, which means $\boldsymbol{\beta}^0 = \mathbf{0}$ (we and the book will do this), and
...
It is convenient to use category $0$ as a **baseline** because we can get logits (log-odds) using some simple algebra on the previous equation:
...
In the case of **ordinal logistic regression**, it changes a little bit. Assume $\mathbf{x}_i$ doesn't have an intercept term. Each group will get it's own intercept:
...
NB: "logistic link" should be replaced with "logit link"
...
so the variance is the negative of the inverse of this matrix above:
$$
...
so an interval for the mean response is 
$$
[g^{-1}\left(\mathbf{x}_0^\intercal \hat{\boldsymbol{\beta}} - z_{\alpha/2}\sqrt{\text{V}(\mathbf{x}_0^\intercal\hat{\boldsymbol{\beta}})} \right) , g^{-1}\left(\mathbf{x}_0^\intercal \hat{\boldsymbol{\beta}} + z_{\alpha/2}\sqrt{\text{V}(\mathbf{x}_0^\intercal\hat{\boldsymbol{\beta}})} \right)]
$$
