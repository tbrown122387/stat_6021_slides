...
$$
\nabla \ell(\boldsymbol{\beta}) = \sum_{i=1}^n y_i \mathbf{x}_i - \frac{1}{( 1 + \exp[ \mathbf{x}_i^\intercal \boldsymbol{\beta}] ) }\mathbf{x}_i  \overset{\text{set}}{=} \mathbf{0}
$$
...
$$
\log\left(\frac{ \tilde{\pi}}{1-\tilde{\pi}} \bigg/  \frac{\pi}{1-\pi} \right) = \beta_3 ( x_{3} - \tilde{x}_{3} ) 
$$
...
A **goodness-of-fit** test uses the same methodology to test your chosen model, which is assumed true under the null hypothesis, against a **saturated model**.

To find the saturated model, we give each row it's own coefficent. So $\boldsymbol{\beta} \in \mathbb{R}^n$ and the design matrix is
$$
...
looking at $\beta_j$:
$$
\sum_{i=1}^n y_i x_{i,j} = \sum_{i=1}^n\frac{1}{( 1 + \exp[ \mathbf{x}_i^\intercal \boldsymbol{\beta}] ) }x_{i,j}  
$$


which can only be true if for each observation $i$:
$$
y_i = \frac{1}{( 1 + \exp[ \beta_i ] ) }
$$

or in other words each $\beta_i$ is plus or minus infinity (if $y_i$ is $1$ or $0$, respectively. Verify that the maximized log-likelihood is $0$!
...
- $\ell( \hat{\boldsymbol{\beta}} )$ is the log-likelihood with the MLE plugged in
...
$$
\nabla \ell(\boldsymbol{\beta}) = \sum_{i=1}^n y_i \mathbf{x}_i - \frac{1}{( 1 + \exp[ \mathbf{x}_i^\intercal \boldsymbol{\beta}] )) }\mathbf{x}_i 
$$
...
Why is there a **null deviance**, and a **residual deviance**?
